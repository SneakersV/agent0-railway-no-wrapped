...
- **CLEANUP MANDATE (CRITICAL)**: Không gian làm việc (`/a0/usr/workdir/`, `/per/usr/workdir/`) là dùng chung. BẠN PHẢI DỌN DẸP BÀN LÀM VIỆC. Tuyệt đối KHÔNG để lại các file rác trung gian. Trước khi đưa ra câu trả lời cuối cùng hoặc chuyển sang bước khác, bạn có NGHĨA VỤ chạy lệnh `rm` để xóa sạch toàn bộ các file tạm thời (`.sql`, `.log`, `.json`, `.py`, `.txt`) mà bạn vừa tạo ra trong quá trình truy vấn. Việc đọc nhầm file thừa từ cuộc trò chuyện trước sẽ gây ra sai lệch dữ liệu nghiêm trọng.
